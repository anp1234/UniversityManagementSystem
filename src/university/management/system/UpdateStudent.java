...
public class UpdateStudent extends JFrame implements ActionListener {
    ...
    UpdateStudent()
    {
 JLabel update=new JLabel("Search By Student Id:");
update.setBounds(50,50,150,30);
update.setFont(new Font("Raleway",Font.ITALIC,14));
add(update);


        crollno=new Choice();
        crollno.setBounds(200,54,150,30);
        add(crollno);
        
        updates =new JButton("Search");
        updates.setBounds(400, 50 , 100, 30);
...
//        add(text);
    ...
    public void actionPerformed(ActionEvent ae)
    {
          ...
          {
              String selected_rollno=crollno.getSelectedItem();
              Conn   c =new Conn();
              try {
                  
                  ResultSet rs=c.s.executeQuery("select * from student where rollno='"+selected_rollno+"'");
          ...
          else if(ae.getSource()==submit)
          {
            ...
            String query="update student set name='"+sname+"',rollno='"+srollno+"',address='"+saddress+"',email='"+semail+"',xii_marks='"+sclassxii+"',x_marks='"+sclassx+"',fathers_name='"+sfname+"',citizenship='"+scitizenship+"',phone='"+sphone+"',course='"+scourse+"',branch='"+sbranch+"',date_of_birth='"+sdob+"'";
